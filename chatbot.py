...
similarity_scores = {}
sub_category_display = None
custom_similarity_scores = {}
custom_sub_category_display=None
...
class QuizBot:
    ...
    def text_to_speech(text):
        if text:
            ...
            return None
    @staticmethod
    def report_generation(similarity_scores):
        summed_scores = {tech: sum(scores) for tech, scores in similarity_scores.items()}
        total_score = sum(summed_scores.values())
        total_possible_score = sum(len(scores) * 10 for scores in similarity_scores.values())
        total_score_percentage = (total_score / total_possible_score) * 100 if total_possible_score > 0 else 0

        individual_scores = {tech: [f"{tech} Question {i + 1}: {score:.1f}" for i, score in enumerate(scores)] for tech, scores in similarity_scores.items()}
        aggregate_scores = {tech: f"{tech}: {score:.1f}" for tech, score in summed_scores.items()}

        return individual_scores, aggregate_scores, total_score, total_score_percentage
...
#             audio_base64 = ''
#         return jsonify({
#             "sub_category_display":sub_category_display,
#             "question": question,
...
def get_question():
    ...
    if request.method == 'POST':
        ...
        level_type = data['level_type']
        try:
            response = requests.post('https://www.interviewaxis.com/api/v1/qa')
            if response.status_code == 200:
                posts = response.json()
                filtered_data = [d for d in posts['data'] if d['childcategory'] in sub_category 
                                 and d['level'].lower() == level_type.lower()]
                if not filtered_data:
                    return jsonify({"error": "No questions found for the selected sub-category and level type."}), 404
                random_dict = random.choice(filtered_data)
                correct_answer = random_dict.get('answer')
                question = random_dict.get('question')
                audio = QuizBot.text_to_speech(question)
                if audio:
                    audio_base64 = base64.b64encode(audio).decode('utf-8')
                else:
                ...
                return jsonify({"message": "No response"}),400
...
def check_answer():
    global similarity_scores, sub_category_display,similarity_score
    data = request.json
    ...
    # Store the similarity score for the current sub-category
    if sub_category_display not in similarity_scores:
        similarity_scores[sub_category_display] = 0
    similarity_scores[sub_category_display] += similarity_score
...
@app.route('/defaut_report',methods=['POST'])
def default_report():
    global similarity_scores, sub_category_display,similarity_score
    if sub_category_display not in similarity_scores:
        similarity_scores[sub_category_display] = 0
    ...
    return jsonify({
        ...
        "category_scores": similarity_scores
    })
...
def upload_JD():
    ...
    if request.method=="POST":
        ...
        # Rest of your logic...
        response = requests.post('https://www.interviewaxis.com/api/v1/childcategory')
        unique_words = []
        if response.status_code == 200:
            posts = response.json()
            unique_words = [d['name'] for d in posts['data']]
            sub_child_dic = { v['name'] : v['id'] for v in posts['data'] }
            ...
            sub_child_ids = [x for k, x in sub_child_dic.items() if k in partial_matches] 
            if 0 <= level <= 3:
                level_type = 'Basic'
            elif 3 < level <= 7:
            ...
            'partial_matches': list(sub_child_ids)
        }), 200
    else:
        return jsonify({"message": "No response"}),400
...
def next_question():
    global level_type, partial_matches, df,sub_category_display
    ...
    data = request.json
    selected_technologies = data.get("selected", [])
    print('Selected Technologies:', selected_technologies)
    sub_category = random.choice(selected_technologies)
...
def custom_check_answer():
    global custom_similarity_scores, sub_category_display,custom_similarity_score
    data = request.json
    user_answer = data['user_answer']
    correct_answer = data['correct_answer']
    similarity_score = QuizBot.compute_similarity(user_answer, correct_answer)
    custom_similarity_score = round(similarity_score * 10, 1)
    ...
    return jsonify({
        "user_answer": user_answer,
        "correct_answer": correct_answer,
        "similarity_score": custom_similarity_score
        
    })
@app.route('/custom_report',methods=['POST'])
def custom_report():
    global custom_similarity_scores, sub_category_display,similarity_score
    if sub_category_display not in custom_similarity_scores:
        custom_similarity_scores[sub_category_display] = 0
    custom_similarity_scores[sub_category_display] += custom_similarity_score
    total_similarity_score = sum(similarity_scores.values())
    return jsonify({
        "total_similarity_score": total_similarity_score,
        "category_scores": similarity_scores
